...
for required in "$source_frame" "$font_display" "$font_ui" "${models[@]}"; do
	[[ -f "$required" ]] || { echo "Missing required source: $required" >&2; exit 1; }
done

mkdir -p "$output_dir"

render_dir="$(mktemp -d "${TMPDIR:-/tmp}/skyyrose-bart-film.XXXXXX")"
[[ -n "$render_dir" && -d "$render_dir" ]] || { echo "Unable to create render directory" >&2; exit 1; }
...
magick "$source_frame" -crop 802x300+0+1325 +repage \
	...
	-colorspace sRGB -blur 0x5 -brightness-contrast -28x12 \
	"$render_dir/background.png"
...
for index in 0 1 2 3 4; do
	...
	magick "$render_dir/background.png" \
		...
		\( "${models[$index]}" -resize '650x975>' \) -gravity northeast -geometry +65+52 -composite \
		-font "$font_ui" -fill '#c9a85e' -pointsize 28 -annotate +132+132 'SKYYROSE / JERSEY SERIES' \
		-font "$font_display" -fill white -pointsize 98 -annotate +132+245 'THE BAY LINE' \
		...
		-font "$font_display" -fill '#d6d0c7' -pointsize 58 -annotate +132+525 'Ride every number home.' \
		-font "$font_ui" -fill '#c9a85e' -pointsize 22 -annotate +132+635 'VERIFIED SOT ON-MODEL MEDIA' \
		...
		-fill '#c9a85e' -pointsize 24 -annotate +132+900 "CURRENT STOP / ${chapters[$index]}" \
		"$frame"
done

ffmpeg -hide_banner -loglevel error -y \
	-loop 1 -t 3.6 -i "$render_dir/frame-0.png" \
	-loop 1 -t 3.6 -i "$render_dir/frame-1.png" \
	-loop 1 -t 3.6 -i "$render_dir/frame-2.png" \
	-loop 1 -t 3.6 -i "$render_dir/frame-3.png" \
	...
	-map "[outv]" -an -r 30 -t 16 \
	-c:v libx264 -preset slow -crf 20 -movflags +faststart \
	"$output_dir/jersey-series-bart-journey.mp4"

ffmpeg -hide_banner -loglevel error -y \
	-i "$output_dir/jersey-series-bart-journey.mp4" -an \
	-c:v libvpx-vp9 -crf 34 -b:v 0 -row-mt 1 \
	"$output_dir/jersey-series-bart-journey.webm"

ffmpeg -hide_banner -loglevel error -y \
	-ss 1.2 -i "$output_dir/jersey-series-bart-journey.mp4" -frames:v 1 \
	-vf "scale=1280:-2" "$render_dir/poster.png"
magick "$render_dir/poster.png" -quality 86 "$output_dir/jersey-series-bart-poster.webp"

ffprobe -v error -show_entries format=duration,size -show_entries stream=codec_name,width,height,r_frame_rate -of default=noprint_wrappers=1 "$output_dir/jersey-series-bart-journey.mp4"
