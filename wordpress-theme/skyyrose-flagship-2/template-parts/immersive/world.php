...
<main id="main-content" class="sr2-immersive sr2-immersive--<?php echo esc_attr( $world_type ); ?>"
	...
	<div class="sr2-immersive__chapters">
		...
		if ( ! $product || 'publish' !== $product->get_status() || ! $product->is_visible() || empty( $product_media[0]['id'] ) ) {
			continue;
		}
			...
			continue;
						...
						<a class="sr2-immersive__hotspot" href="<?php echo esc_url( get_permalink( $product->get_id() ) ); ?>"
							...
							<span class="sr2-immersive__hotspot-media">
								<?php echo wp_kses_post( wp_get_attachment_image( absint( $product_media[0]['id'] ), 'woocommerce_thumbnail', false, array( 'class' => 'sr2-immersive__hotspot-image', 'loading' => 'lazy', 'decoding' => 'async', 'alt' => $product->get_name() ) ) ); ?>
							</span>
