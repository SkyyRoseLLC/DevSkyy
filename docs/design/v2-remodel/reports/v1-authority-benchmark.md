...
| Browser, responsive, screen-reader, payment, order, and independent visual evidence | Not present in the historical V2 seed; current same-candidate evidence remains incomplete | UNVERIFIED |
...
|---|---|---|---|
| Each collection is its own story-world experience. | `skyyrose2_collections()` declares four collection-specific narrative records; `template-collection.php` renders a hero, product section, scenic story rail, manifesto, and cross-navigation per collection. Scroll World code provides a desktop pinned treatment with native-rail fallback. | Each story asset needs an explicit current identity/manifest/right record, local package resolution, and 390/768/1440 visual evidence. The historical seed lacked a candidate manifest; the active candidate now has one, but independent review remains unresolved. | OBSERVED / UNVERIFIED |
...
Planning: V2 gap-closure procedure, V2 page-and-imagery plan, and Fashion Theme Brain taxonomy/merchandising pack. The historical prototype worktree lacked `.wolf/memory.md` and `docs/design/fashion-design-system-team.md`; the active repository is governed by the current root instructions and `.wolf/memory.md`.
