...
try:
    ...
    logging.warning(
        "JWT authentication module not available - endpoints will be unprotected")
...
async def generate_virtual_tryon(
    request: VirtualTryOnRequestModel, background_tasks: BackgroundTasks
):
    """
    ...
    try:
        ...
        gen_request = GenerationRequest(
            ...
            style_preset=StylePreset(
                request.style_preset) if request.style_preset else None,
            provider=ContentProvider(
                request.provider) if request.provider else None,
            width=request.width,
    ...
    return {
        ...
        "agents": {
            "visual_content": {
                ...
                "status": (
                    visual_content_agent.get_system_status() if VISUAL_AGENT_AVAILABLE else None
                ),
            },
            "finance_inventory": {
                "available": FINANCE_AGENT_AVAILABLE,
                "status": (
                    finance_inventory_agent.get_system_status() if FINANCE_AGENT_AVAILABLE else None
                ),
            },
            "marketing": {
                "available": MARKETING_AGENT_AVAILABLE,
                "status": (
                    marketing_orchestrator.get_system_status()
                    if MARKETING_AGENT_AVAILABLE
                    else None
                ),
            },
            "code_development": {
            ...
            "workflow_engine": {
                ...
                "status": (
                    workflow_engine.get_system_status() if WORKFLOW_ENGINE_AVAILABLE else None
                ),
            },
        },
    }
