...
async def get_performance_metrics(current_user: Dict = Depends(get_current_user)):
    ...
    try:
        ...
        cache_hit_rate = (
            metrics_collector.cache_hits / total_cache_ops if total_cache_ops > 0 else 0
        )

        total_webhooks = metrics_collector.webhook_successes + metrics_collector.webhook_failures
        webhook_success_rate = (
            metrics_collector.webhook_successes /
            total_webhooks if total_webhooks > 0 else 0)

        total_agents = metrics_collector.agent_successes + metrics_collector.agent_failures
        agent_success_rate = (
            metrics_collector.agent_successes / total_agents if total_agents > 0 else 0
        )
...
async def get_active_alerts(
    severity: Optional[str] = None, current_user: Dict = Depends(get_current_user)
):
    """
...
async def check_dependencies(current_user: Dict = Depends(get_current_user)):
    ...
    dependencies = {
        ...
        # TODO: Ping Redis
        "cache": {"status": "unknown", "latency_ms": None, "error": None},
        "anthropic_api": {
