...
class VercelBuildMonitor:
    ...
    def check_python_imports(self):
        ...
        try:
            ...
            if found_heavy:
                ...
                logger.info(
                    "   Consider lazy loading or removing for better cold starts")
            else:
    ...
    def generate_report(self):
        ...
        logger.info(
            f"Dependencies: {self.metrics['dependencies'].get('count', 'Unknown')}")
        logger.info(
            f"Total Size: {self.metrics['performance'].get('total_size', 0) / (1024*1024):.2f} MB")
        logger.info(
            f"Cold Start: {self.metrics['performance'].get('cold_start_time', 0):.3f} seconds")
