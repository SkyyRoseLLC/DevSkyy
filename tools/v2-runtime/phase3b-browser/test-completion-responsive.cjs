...
const routes = {
  home: '/', signature: '/collections/signature/', 'black-rose': '/collections/black-rose/',
  'love-hurts': '/collections/love-hurts/', kids: '/collections/kids-capsule/',
...
(async () => {
  ...
  try {
    for (const width of widths) {
      ...
      let errors = [], httpErrors = [];
      page.on('pageerror', error => errors.push(error.message));
      page.on('response', response => { if (response.status() >= 400 && response.request().resourceType() !== 'document') httpErrors.push({ url: response.url(), status: response.status() }); });
      for (const [name, route] of Object.entries(routes)) {
        ...
        const state = await page.evaluate(() => ({
          ...
          h1: [...document.querySelectorAll('main h1')].map(el => ({ text: el.textContent.trim(), top: el.getBoundingClientRect().top, bottom: el.getBoundingClientRect().bottom })),
          main: !!document.querySelector('main'),
          errorText: /Fatal error|Uncaught (?:Error|Exception)|Warning: (?:Undefined|require|include)/.test(document.body.innerText),
          missingImages: [...document.querySelectorAll('main img')].filter(img => img.complete && img.currentSrc && !img.naturalWidth).map(img => img.currentSrc)
        }));
        let violations = [];
        if ([390, 1440].includes(width)) {
          const result = await new (q('@axe-core/playwright').default)({ page }).withTags(['wcag2a', 'wcag2aa', 'wcag21aa', 'wcag22aa']).analyze();
        ...
        const row = { name, width, url: page.url(), status: response.status(), state, errors: [...errors], httpErrors: [...httpErrors], violations, screenshot };
        const expectedStatus = name === 'missing' ? 404 : 200;
        if (row.status !== expectedStatus || !state.main || state.h1.length !== 1 || state.scrollWidth > width || state.errorText || state.missingImages.length || row.errors.length || row.httpErrors.length || violations.length) evidence.failures.push(row);
...
})().catch(error => { run.fail(error, evidence); console.error(error); process.exitCode = 1; });
