...
const source = fs.readFileSync(path.resolve(__dirname, '../../../wordpress-theme/skyyrose-flagship-2/assets/js/premium-commerce.js'), 'utf8');
...
async function run(action) { const browser=await chromium.launch(); try { const page=await browser.newPage(); await page.route('http://premium.test/**',r=>r.fulfill({contentType:'text/html',body:fixture}));await page.route('**/*.svg',r=>r.request().url().includes('broken')?r.abort():r.fulfill({contentType:'image/svg+xml',body:'<svg xmlns="http://www.w3.org/2000/svg" width="20" height="10"><rect width="20" height="10" fill="black"/></svg>'}));await page.goto('http://premium.test');await page.addScriptTag({content:source});await action(page);}finally{await browser.close();}}
test('poster previews follow menu, focus and explicit touch button without hijacking links',()=>run(async page=>{
...
test('image failure is honest, preview errors retain destinations and initialization is singular',()=>run(async page=>{
 ...
 await page.evaluate(()=>document.querySelectorAll('.sr2-house-nav__preview-entry img')[1].src='/poster-1.svg?failure=1');
...
test('native card request tracking isolates concurrent errors and clears only originating busy state',()=>run(async page=>{
 const result=await page.evaluate(async code=>{
   const handlers={};
   window.jQuery=()=>({on(names,handler){handlers[names]=handler;return this;}});
   document.documentElement.removeAttribute('data-sr2-premium-initialized');
   const script=document.createElement('script');script.textContent=code;document.head.append(script);
   // Exercise actual handlers with distinct native event records; no product/cart mutation is invented.
   const cards=[1,2].map(id=>{const c=document.createElement('article');c.className='sr2-c-editorial-card';c.innerHTML=`<button class="loading" aria-busy="true" data-product_id="${id}">Add</button><p data-card-cart-feedback data-added="Added" data-failed="Try the product" hidden></p>`;document.body.append(c);return c;});
   ...
   const exact=cards[0].dataset.cartState==='loading'&&cards[1].dataset.cartState==='error'&&buttons[0].classList.contains('loading')&&!buttons[1].classList.contains('loading')&&!buttons[1].hasAttribute('aria-busy');
   handlers['added_to_cart.sr2Premium']({}, {},'', [buttons[0]]);
   return {unrelatedSafe,exact,success:cards[0].dataset.cartState==='success',errorText:cards[1].querySelector('p').textContent};
 },source);
 assert.equal(result.unrelatedSafe,true);assert.equal(result.exact,true);assert.equal(result.success,true);assert.equal(result.errorText,'Try the product');
}));
