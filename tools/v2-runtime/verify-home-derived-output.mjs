...
import { covered } from './css-class-coverage.mjs';
...
if (!base) {
  console.error('--base=https://host is required');
  process.exit(2);
}
...
const fetchText = async url => {
  const r = await fetch(url, {
    headers: { 'user-agent': 'Mozilla/5.0 (Macintosh) Chrome/128 skyyrose-derived-output' },
  });
...
report.document = {
  ...
  cache: home.headers['x-ac'] || home.headers['x-cache'] || null,
};
if (home.status !== 200) failures.push(`Home returned HTTP ${home.status}`);
const headEnd = html.indexOf('</head>');

// 1. Theme critical contract.
const localCritical = (await fs.readFile(path.join(themeDir, 'assets/css/critical/home.min.css'), 'utf8'))
  .trim()
  .replaceAll('__SKYYROSE2_ASSETS__', `${themeUri}/assets`);
const criticalMatch = html.match(/<style id="skyyrose2-critical-home">([\s\S]*?)<\/style>/);
report.themeCritical = {
  present: Boolean(criticalMatch),
  bytes: criticalMatch ? criticalMatch[1].length : 0,
  inHead: criticalMatch ? html.indexOf(criticalMatch[0]) < headEnd : false,
  matchesLocal: criticalMatch ? criticalMatch[1].trim() === localCritical : false,
};
...
report.heroBootstrap = {
  present: Boolean(bootstrapMatch),
  boostIgnore: bootstrapMatch ? /data-jetpack-boost="ignore"/.test(bootstrapMatch[1]) : false,
  matchesLocal: bootstrapMatch ? bootstrapMatch[2].trim() === localController : false,
  afterHero: bootstrapMatch
    ? html.indexOf(bootstrapMatch[0]) > heroEnd && html.indexOf(bootstrapMatch[0]) - heroEnd < 200
    : false,
  footerCopies: (html.match(/visual-recovery\.min\.js/g) || []).length,
};
if (!report.heroBootstrap.present) failures.push('inline hero bootstrap missing');
else {
  if (!report.heroBootstrap.boostIgnore) failures.push('inline hero bootstrap lacks data-jetpack-boost="ignore"');
  if (!report.heroBootstrap.matchesLocal) failures.push('inline hero bootstrap differs from the local controller');
  if (!report.heroBootstrap.afterHero) failures.push('inline hero bootstrap is not directly after the hero');
}
if (report.heroBootstrap.footerCopies > 0)
  failures.push('a footer copy of the hero controller is still delivered on Home');

// 3. Served theme files match local build.
const files = [
  'assets/css/design-tokens.min.css',
  'assets/css/theme.min.css',
  'assets/css/global-shell.min.css',
  'assets/css/home-page.min.css',
  'assets/css/controls.min.css',
  'assets/css/visual-recovery.min.css',
  'assets/css/collection-world.min.css',
  'assets/css/mascot.min.css',
  'assets/js/theme.min.js',
  'assets/js/visual-recovery.min.js',
  'assets/js/mascot-loader.min.js',
  'assets/css/critical/home.min.css',
];
report.files = [];
...
for (const m of firstView.matchAll(/class="([^"]*)"/g))
  for (const c of m[1].split(/\s+/)) if (/^(sr2|skyyrose)-/.test(c)) classes.add(c);
const navStart = firstView.indexOf('<nav id="sr2-menu"');
const navEnd = firstView.indexOf('</nav>', navStart);
const navClasses = new Set();
if (navStart !== -1)
  for (const m of firstView.slice(navStart, navEnd).matchAll(/class="([^"]*)"/g))
    for (const c of m[1].split(/\s+/)) navClasses.add(c);
...
report.coverage = {
  firstViewClasses: classes.size,
  uncoveredByContract: uncovered,
  boost: {
    present: Boolean(boostMatch),
    bytes: boostCss.length,
    uncovered: [...classes].filter(c => !navClasses.has(c) && !covered(boostCss, c)).length,
  },
};
...
const scripts = [...html.matchAll(/<script\b([^>]*)>/gi)].map((m, i) => ({
  i,
  attrs: m[1],
  src: (m[1].match(/\ssrc=["']([^"']+)/) || [])[1] || '',
  id: (m[1].match(/\sid=["']([^"']+)/) || [])[1] || '',
}));
const bootstrapIndex = scripts.findIndex(s => s.id === 'skyyrose2-visual-recovery-early');
const headScripts = scripts
  .filter(s => html.indexOf(`<script${s.attrs}>`) < headEnd && s.src)
  .map(s => s.src.split('/').pop());
// The hero bootstrap must precede every classic body script it used to wait behind.
const laterChain =
  /jquery-migrate|underscore|wp-util|add-to-cart-variation|_jb_static|theme\.min\.js|jquery(\.min)?\.js/;
const firstChainIndex = scripts.findIndex(s => laterChain.test(s.src) && html.indexOf(`<script${s.attrs}>`) > headEnd);
report.scriptOrder = {
  bootstrapIndex,
  firstBodyChainIndex: firstChainIndex,
  headScripts,
  bootstrapBeforeBodyChain: bootstrapIndex !== -1 && (firstChainIndex === -1 || bootstrapIndex < firstChainIndex),
};
if (!report.scriptOrder.bootstrapBeforeBodyChain)
  failures.push('hero bootstrap is not delivered before the classic body script chain');

// 6. First-view font preloads equal the inline @font-face URLs.
const faceUrls = [...contractCss.matchAll(/@font-face\{[^}]*?src:url\(["']?([^"')]+)["']?\)/g)]
  .map(m => m[1])
  .filter(u => /archivo-latin|hanken-grotesk-latin|anton-latin|cinzel-latin/.test(u));
const preloadLinks = [...html.slice(0, headEnd).matchAll(/<link\b([^>]*rel=["']preload["'][^>]*)>/gi)].map(m => m[1]);
const fontPreloads = preloadLinks
  .filter(a => /as=["']font["']/.test(a))
  .map(a => ({ href: (a.match(/href=["']([^"']+)/) || [])[1] || '', crossorigin: /crossorigin/.test(a) }));
report.fontPreloads = { expected: faceUrls, present: fontPreloads };
for (const url of faceUrls) {
  const hit = fontPreloads.find(p => p.href === url);
  if (!hit) failures.push(`first-view font is not preloaded with its @font-face href: ${url}`);
  else if (!hit.crossorigin) failures.push(`font preload lacks crossorigin: ${url}`);
}
...
if (failures.length > 0) {
  console.error(`\nDERIVED OUTPUT PARITY: FAIL (${failures.length})`);
  process.exit(1);
}
