...
function projectTokens(current, tokens) {
  ...
  mapEntries(next.settings.color.palette, {
    'concrete-black':'void',surface:'surface-commerce',white:'white',
    'text-secondary':'color-text','text-muted':'color-text-muted',border:'border-solid',
    'rose-gold':'rose','signature-gold':'gold','black-rose-silver':'silver','love-hurts-crimson':'crimson'
  }, 'color');
  mapEntries(next.settings.spacing.spacingSizes, {
    '2-xs':'space-1',xs:'space-2',small:'space-4',medium:'space-6',large:'space-10',xl:'space-16','2-xl':'space-24'
  }, 'size');
  mapEntries(next.settings.typography.fontFamilies, {
    archivo:'font-display','hanken-grotesk':'font-body',anton:'font-ui',cinzel:'font-caps'
  }, 'fontFamily');
  mapEntries(next.settings.typography.fontSizes, {
    small:'type-utility',body:'type-body',lead:'type-editorial','display-small':'type-editorial',display:'type-display'
  }, 'size');
  for (const role of ['monument','commerce','index']) {
    ...
    if (existing) Object.assign(existing, entry); else next.settings.typography.fontSizes.push(entry);
  }
  ...
  Object.assign(next.settings.custom.focus, {
    color:token('focus-outer'),inner:token('focus-inner'),width:token('focus-width'),offset:token('focus-offset')
  });
  next.settings.custom.controls = {size:token('control-size'),target:token('target-size'),border:token('border-control')};
  next.settings.custom.layout = {reading:token('reading'),commerce:token('commerce-width'),gap:token('grid-gap')};
  next.settings.custom.typeRoles = Object.fromEntries(['monument','display','editorial','commerce','body','utility','index'].map(role => [role, {
    size:token(`type-${role}`),
    family:token(role === 'index' ? 'font-index' : ['monument','display','editorial'].includes(role) ? 'font-display' : role === 'utility' ? 'font-ui' : 'font-body'),
