...
function generateFromTemplate(
  ...
  const baseHashtags = ['#SkyyRose', '#LuxuryStreetwear', collectionTag];

  const templates: Record<string, Record<string, string>> = {
    product_launch: {
      instagram: `Introducing ${product.label} from the ${collectionLabel} collection.\n\nThis piece was crafted for those who dare to stand out. Every stitch tells a story of bold elegance.\n\nAvailable now at skyyrose.co\n\n${baseHashtags.join(' ')} #NewDrop #FashionForward`,
      ...
      twitter: `${product.label} is here. ${collectionLabel} collection. \n\nskyyrose.co ${baseHashtags.slice(0, 2).join(' ')}`,
...
The brand has no active tagline. Do not add a tagline.
