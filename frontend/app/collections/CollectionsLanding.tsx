...
const RotatingLogoFallback = lazy(() => import('@/components/3d/RotatingLogoFallback'));
    ...
    <>
      ...
      <header className='relative pt-32 pb-16 px-4 text-center'>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <h1 className='text-5xl md:text-7xl font-display text-white mb-6'>Collections</h1>
          <p className='text-white/40 max-w-lg mx-auto text-sm leading-relaxed'>
            Each collection is a portal into a different universe of luxury fashion. Explore AI-crafted designs,
            immersive 3D scenes, and exclusive pieces waiting to be yours.
          </p>
        </motion.div>
              ...
              >
                ...
                <div className='absolute inset-0 flex items-end p-6 md:p-12'>
                  <div className='flex-1'>
                    <p className='text-xs tracking-[0.3em] uppercase mb-2' style={{ color: collection.accentColor }}>
                      {collection.tagline}
                    </p>
                    ...
                    <div className='flex gap-4 text-white/25 text-xs tracking-wider'>
                      ...
                      <span>&middot;</span>
                      <span>
                        {collection.scenes.length} {collection.scenes.length === 1 ? 'scene' : 'scenes'}
                      </span>
                      <span>&middot;</span>
                  ...
                  <div className='hidden lg:block w-[250px] shrink-0'>
                    <Suspense fallback={null}>
                      <RotatingLogoFallback text={collection.name.charAt(0)} color={collection.accentColor} />
                    </Suspense>
                  </div>
                </div>
              </Link>
      ...
      <section className='py-20 px-4 text-center border-t border-white/5'>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <p className='text-[#B76E79] text-xs tracking-[0.3em] uppercase mb-4'>Limited Availability</p>
          <h2 className='text-3xl md:text-4xl font-display text-white mb-4'>Ready to Own the Future?</h2>
          <p className='text-white/30 text-sm mb-8 max-w-md mx-auto'>
            Pre-order from all three collections and join the SkyyRose Inner Circle.
          </p>
          <Link
            href='/pre-order'
      ...
      <footer className='border-t border-white/5 py-12 px-4 text-center pb-24'>
        <p className='text-white/20 text-xs tracking-[0.2em] uppercase'>Luxury Grows from Concrete.</p>
        <p className='text-white/10 text-xs mt-2 tracking-wider'>SkyyRose LLC &copy; 2026</p>
      </footer>
...
function CollectionCardImage({
  ...
  if (heroImageAvailable === false || imageError) {
    return (
      ...
      >
        <span className='text-8xl font-display opacity-20' style={{ color: accentColor }}>
          {name}
        </span>
      </div>
    );
