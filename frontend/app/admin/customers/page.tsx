...
export default async function CustomersPage() {
  // Establish the request-time boundary before reading the NextAuth cookie.
  // Without this, Cache Components can prerender the page without the owner
  // session and leave the authenticated console looking like a dead route.
  await connection();
  const session = await getAdminSession();
  const [customers, orders] = session ? await Promise.all([safeCustomers(), safeOrders()]) : [[], []];
        ...
        <ConsoleCard className='p-6'>
          ...
          {customers.map(c => {
            ...
            return (
              <ConsoleRow
                key={c.id}
                className='grid gap-3 px-2 py-3.5 border-b border-white/[0.04] items-center'
                style={{ gridTemplateColumns: '2fr 1fr 1.4fr 80px 110px' }}
              >
