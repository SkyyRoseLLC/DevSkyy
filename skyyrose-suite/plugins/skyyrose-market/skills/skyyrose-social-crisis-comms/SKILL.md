...
# Generate a holding statement for a Love Hurts pre-order delay (L2 crisis)
...
# Generate a full public statement for a product quality issue (L3)
...
```bash
# Smoke-test crisis comms content type via the social venture
...
```json
// Crisis log entry schema (append to crisis-log.json for every L2+ incident)
{
...
- **Negative sentiment persists weeks after resolution:** Monitor via `skyyrose-social-social-listening-plan`. The only recovery is consistent follow-through on every committed change. If we said we were fixing the 3PL, the next drop ships on time and we say so. Actions rebuild the trust; statements alone do not.
