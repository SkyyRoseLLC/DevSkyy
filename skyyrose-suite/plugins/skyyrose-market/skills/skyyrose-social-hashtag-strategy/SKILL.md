...
- Building the hashtag bank referenced in `skyyrose-social-social-media-calendar` slots
...
- Fixing a discoverability gap identified in a `skyyrose-social-social-media-audit`
...
- **No banned, spammy, or generic-engagement hashtags** — `#instagood`, `#like4like`, `#followback` are disqualified on brand grounds, not just algorithmic ones.
...
# Generate a post with collection-aware hashtags built in
post = agent.generate_post("br-001", "instagram", "product_launch")
print(post.caption)
print("--- First comment hashtags ---")
print(post.hashtags)

# Generate for Reels (fewer hashtags)
...
```bash
# Smoke test generates posts with hashtags for validation:
...
- **Mixing collection hashtags on one post** — A single post cannot be both `#BlackRoseCollection` and `#LoveHurtsCollection`. Collections are emotionally distinct. Mixed tags signal brand confusion.
...
- **Generic engagement tags** — `#instagood`, `#like4like`, `#follow4follow` are brand-quality violations, not just algorithmic dead weight. They make SkyyRose look like a Canva brand.
