...
# Pull Signature collection voice + hashtags for the shared content brief
ctx = agent.get_collection_context("signature")
print(ctx["mood"])        # register both partners' content must respect
print(ctx["hashtags"])    # SkyyRose hashtag set for the cross-promotion schedule

# A product blurb in the right voice for the partner content brief
post = agent.generate_post("sg-001", "instagram", "product_launch")
print(post.caption)       # the SkyyRose voice the partner adapts to (never co-brands over)
```

```bash
# Inspect the social venture surface backing the partnership:
