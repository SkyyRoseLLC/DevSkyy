...
# Load the collection-correct voice + hashtags for the brief
ctx = agent.get_collection_context("black-rose")
print(ctx["mood"])        # the register the creator must stay inside
print(ctx["hashtags"])    # collection hashtag set for the brief's required-tags + FTC section

# A product blurb in the right voice for the brief's messaging block
post = agent.generate_post("br-001", "instagram", "product_launch")
print(post.caption)       # messaging block — the creator adapts, never copies verbatim
```

```bash
# Inspect the social venture surface that the brief plugs into:
