...
class WebhookManager:
    ...
    async def emit(self, event_type: WebhookEvent, data: Dict[str, Any]):
        ...
        payload = WebhookPayload(
            event_type=event_type, timestamp=datetime.now(timezone.utc), data=data
        )
        ...
        if tasks:
            ...
            logger.debug(
                f"Event '{event_type}' emitted to {len(matching_subscriptions)} subscribers"
            )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=subscription.timeout_seconds) as client:
                response = await client.post(
                    str(subscription.endpoint), content=payload_json, headers=headers
                )
            ...
            if attempt < subscription.max_retries:
                ...
                backoff_seconds = subscription.retry_delay_seconds * \
                    (2 ** (attempt - 1))
    ...
    async def get_delivery_history(
        self, subscription_id: str, limit: int = 10
        ...
        """
        matching = [
            d for d in self.delivery_history if d.subscription_id == subscription_id]
        return matching[-limit:]  # Return most recent

    async def get_subscription(
            self,
            subscription_id: str) -> Optional[WebhookSubscription]:
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        await webhook_manager.emit(
            WebhookEvent.AGENT_COMPLETED, {"agent_id": "scanner", "status": "success"}
        )
