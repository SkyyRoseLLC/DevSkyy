...
def _generate_commit_message(fixes_applied: Dict[str, Any]) -> str:
    ...
    pr_context = fixes_applied.get("pr_context")
    
    if not fixes_applied or not fixes_applied.get("fixes_applied"):
        if pr_context:
            pr_number = pr_context.get("pr_number")
            return f"Auto-fix: PR #{pr_number} - No issues found\n\nGeneral improvements and optimizations for pull request"
    ...
    # Build commit message with PR context if available
    if pr_context:
        pr_number = pr_context.get("pr_number")
        head_branch = pr_context.get("head_branch")
        title = f"Auto-fix: PR #{pr_number} - {len(fixes)} issues resolved"
        message_parts = [title, f"Branch: {head_branch}", ""]
    else:
    ...
    if pr_context:
        message_parts.extend(["", "Auto-generated by DevSkyy Enhanced Platform (PR Auto-Fix)"])
    else:
