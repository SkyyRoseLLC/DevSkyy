...
from agent.modules.frontend.wordpress_fullstack_theme_builder_agent import (
...
class ThemeBuilderOrchestrator:
    ...
    def __init__(self):
        ...
        enterprise_logger.info(
            "Theme builder orchestrator initialized", category=LogCategory.SYSTEM
        )
    ...
    async def build_and_deploy_theme(
            self, request: ThemeBuildRequest) -> ThemeBuildResult:
        ...
        try:
            ...
            result.build_log.append(
                f"[{datetime.now()}] Theme generated at: {theme_path}")
            ...
            if request.auto_deploy:
                ...
                result.build_log.append(
                    f"[{datetime.now()}] Deploying theme to {request.target_site}"
                )
                ...
                if deployment_result.success:
                    result.build_log.append(
                        f"[{datetime.now()}] Theme deployed successfully")
                else:
                    raise Exception(
            ...
            result.build_log.append(
                f"[{datetime.now()}] Theme build completed successfully")
            ...
            enterprise_logger.info(
                ...
                metadata={
                    ...
                    "duration": (
                        result.completed_at -
                        result.created_at).total_seconds(),
                },
            )
    ...
    async def _generate_theme(
        self, request: ThemeBuildRequest, result: ThemeBuildResult
    ) -> Optional[str]:
        ...
        try:
            ...
            enterprise_logger.error(
                f"Theme generation error: {e}", category=LogCategory.SYSTEM, error=e
            )
            return None

    async def _generate_theme_files(
        self, theme_dir: Path, config: Dict[str, Any], result: ThemeBuildResult
    ):
    ...
    async def _generate_elementor_widgets(
        self, theme_dir: Path, config: Dict[str, Any], result: ThemeBuildResult
    ):
        ...
        try:
            ...
            for file in required_files:
                if not (theme_dir / file).exists():
                    result.build_log.append(
                        f"[{datetime.now()}] Missing required file: {file}")
                    return False
            ...
            with open(style_css, "r", encoding="utf-8") as f:
                ...
                if "Theme Name:" not in content:
                    result.build_log.append(
                        f"[{datetime.now()}] Invalid style.css header")
                    return False
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "successful_builds": len(
                [r for r in self.build_history if r.status == BuildStatus.COMPLETED]
            ),
            "supported_theme_types": [t.value for t in ThemeType],
            "uploader_status": self.theme_uploader.get_system_status(),
            "available_sites": (
                wordpress_credentials_manager.list_available_sites()
                if wordpress_credentials_manager
                else []
            ),
        }
