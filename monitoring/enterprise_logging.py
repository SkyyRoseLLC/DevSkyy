...
class EnterpriseLogger:
    ...
    def _create_log_entry(
        ...
        context = self.get_context() or LogContext(
            correlation_id=str(
                uuid.uuid4()), environment=os.getenv(
    ...
    def error(
        ...
        """Log error message."""
        entry = self._create_log_entry(
            LogLevel.ERROR, category, message, metadata, error)
    ...
    def critical(
        ...
        entry = self._create_log_entry(
            LogLevel.CRITICAL, category, message, metadata, error)
    ...
    def audit(self, action: str, resource: str,
        ...
        """Log audit event."""
        entry = self._create_log_entry(
            LogLevel.INFO, LogCategory.AUDIT, f"Audit: {action} on {resource}", metadata
        )
    ...
    def security(self, event: str, severity: str = "medium",
                 metadata: Optional[Dict[str, Any]] = None):
        """Log security event."""
        security_metadata = {"severity": severity,
                             "event_type": "security", **(metadata or {})}

        level = LogLevel.WARNING if severity in ["medium", "high"] else LogLevel.INFO
        entry = self._create_log_entry(
            level, LogCategory.SECURITY, f"Security Event: {event}", security_metadata
        )
