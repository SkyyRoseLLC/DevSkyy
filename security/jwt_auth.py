...
pwd_context = CryptContext(
    # NIST recommendation: 10-12 rounds
...
def create_access_token(
    user_id: str, roles: List[str], expires_delta: Optional[timedelta] = None
) -> str:
    """
...
def require_role(required_role: UserRole):
    ...
    async def role_checker(current_user: Dict = Depends(
        ...
        if not user_roles:
            ...
            if not user_role_levels:
                ...
                user_role = next(
                    role for role, level in ROLE_HIERARCHY.items() if level == max_level
                )
...
async def get_current_admin(current_user: Dict = Depends(
        get_current_user)) -> Dict[str, Any]:
    """
