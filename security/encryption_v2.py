...
def encrypt_field(plaintext: str, master_key: Optional[bytes] = None) -> str:
    ...
    try:
        ...
        ciphertext = cipher.encrypt(
            nonce, plaintext.encode("utf-8"), None  # No associated authenticated data
        )
        ...
        logger.debug(
            f"Field encrypted successfully (plaintext length: {len(plaintext)})")
        return encoded
...
def decrypt_field(
        ...
        master_key: Optional[bytes] = None,
        legacy_key_id: Optional[str] = None) -> str:
    """
