...
class SQLiteTestSuite:
    ...
    async def test_model_operations(self) -> bool:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                print(
                    f"   ✓ Created user (ID: {user.id}) and product (ID: {product.id})")
                return True
    ...
    async def test_query_performance(self) -> bool:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                # Query all products
                result = await session.execute(
                    select(Product).where(Product.category == "Performance")
                )
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                brand_asset = BrandAsset(
                    asset_type="color_palette", name="Primary Colors", data={
                        "primary": "#FF6B6B", "secondary": "#4ECDC4", "accent": "#45B7D1"}, asset_metadata={
                        "created_by": "AI Designer", "version": "1.0"}, )
    ...
    def print_summary(self):
        ...
        if passed == total:
            ...
            failed_tests = [
                name for name,
                result in self.test_results.items() if not result]
