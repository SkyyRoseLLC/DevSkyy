...
def _validate_edit_region(
    ...
    if kind == "bbox":
        ...
        raw_path = region.get("mask_path")
        expected_sha = region.get("mask_sha256")
        if not isinstance(raw_path, str) or not raw_path:
            findings.append(
                Finding("error", "EDIT_MASK_PATH_REQUIRED", "mask edit regions require mask_path")
            )
            return
        if not isinstance(expected_sha, str) or not HEX64.fullmatch(expected_sha):
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_SHA256_REQUIRED",
                    "mask edit regions require a lowercase mask_sha256",
                )
            )
            return
        try:
            mask_path = resolve_path(raw_path, workspace)
            mask, actual_sha = load_image_snapshot(mask_path, mode="L")
        except GateError as exc:
            findings.append(Finding("error", "EDIT_MASK_UNREADABLE", str(exc), raw_path))
            return
        if actual_sha != expected_sha:
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_HASH_DRIFT",
                    f"mask SHA-256 drift: expected {expected_sha}, got {actual_sha}",
                    str(mask_path),
                )
            )
        if mask.format != "PNG":
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_FORMAT_INVALID",
                    "mask edit regions require a PNG",
                    str(mask_path),
                )
            )
        if mask.size != size:
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_DIMENSION_DRIFT",
                    f"mask dimensions must equal target dimensions {size}",
                    str(mask_path),
                )
            )
        pixels = np.asarray(mask, dtype=np.uint8)
        if not np.any(pixels == 255):
            findings.append(
                Finding("error", "EDIT_MASK_EMPTY", "mask must contain editable white pixels", str(mask_path))
            )
        if np.any((pixels != 0) & (pixels != 255)):
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_NON_BINARY",
                    "mask must use only black locked pixels and white editable pixels",
                    str(mask_path),
                )
            )
        if region.get("mask_polarity") != "white_is_editable_black_is_locked":
            findings.append(
                Finding(
                    "error",
                    "EDIT_MASK_POLARITY_INVALID",
                    "mask_polarity must be white_is_editable_black_is_locked",
                    str(mask_path),
                )
            )
    else:
        findings.append(Finding("error", "EDIT_REGION_TYPE_INVALID", "Use polygon, bbox, or mask"))
...
def build_edit_mask(
    ...
    *,
    ...
    if region["type"] == "mask":
        raw_path = region["mask_path"]
        path = resolve_path(raw_path, workspace) if workspace is not None else Path(raw_path)
        mask, _ = load_image_snapshot(path, mode="L")
        if mask.size != target_size:
            raise GateError(
                "Explicit edit mask dimensions differ from the target",
                "EDIT_MASK_DIMENSION_DRIFT",
            )
        return mask
...
def optimize_references(
    ...
    if contract["operation"] == "localized_product_patch":
        ...
        mask = build_edit_mask(contract, target_image.size, workspace=workspace)
        mask_path = out_dir / "editable-region-mask-white-is-editable.png"
        mask.save(mask_path, format="PNG")
        openai_mask = build_openai_edit_mask(
            contract, target_image.size, workspace=workspace
        )
...
def verify_generation_receipt(
    ...
    if not isinstance(mask_path_value, str) or not mask_path_value.strip():
        ...
        try:
            ...
            expected_mask = build_openai_edit_mask(
                contract, target.size, workspace=workspace
            )
            mask_report = {
                "path": str(mask_path),
