...
class FidelityGateTests(unittest.TestCase):
    ...
    def test_localized_mask_region_allows_disconnected_authorized_islands(self) -> None:
        contract = self.localized_contract()
        mask_path = self.workspace / "multi-island-mask.png"
        mask = Image.new("L", (1024, 1024), 0)
        draw = ImageDraw.Draw(mask)
        draw.rectangle((16, 16, 47, 47), fill=255)
        draw.rectangle((128, 128, 159, 159), fill=255)
        mask.save(mask_path, format="PNG")
        contract["edit_region"] = {
            "type": "mask",
            "coordinate_space": "pixels",
        ...
        provider_mask = fidelity_gate.build_openai_edit_mask(
            contract,
            (1024, 1024),
            workspace=self.workspace,
        )
        self.assertEqual(0, provider_mask.getpixel((16, 16))[3])
        self.assertEqual(255, provider_mask.getpixel((80, 80))[3])

        candidate = self.workspace / "multi-island-candidate.png"
        image = Image.open(self.target).convert("RGBA")
        image.putpixel((20, 20), (255, 0, 0, 255))
        image.putpixel((140, 140), (0, 255, 0, 255))
        image.save(candidate, format="PNG")
        result = fidelity_gate.verify_localized_output(
            contract,
            self.workspace,
            candidate,
        )
