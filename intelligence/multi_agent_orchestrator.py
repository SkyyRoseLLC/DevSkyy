...
class MultiAgentOrchestrator:
    ...
    def _initialize_providers(self):
        ...
        try:
            ...
            self.providers[AIProvider.OPENAI_GPT4] = OpenAIIntelligenceService(
                model="gpt-4")
            self.providers[AIProvider.OPENAI_GPT35] = OpenAIIntelligenceService(
                model="gpt-3.5-turbo"
            )
    ...
    def get_optimal_provider(self, task: TaskRequest) -> Optional[AIProvider]:
        ...
        suitable_providers.sort(
            key=lambda x: (x[1].performance_score, -x[1].cost_per_request), reverse=True
        )
    ...
    async def _process_with_specialized_provider(
            self, provider_service, task: TaskRequest) -> str:
