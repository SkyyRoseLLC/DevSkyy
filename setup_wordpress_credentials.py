...
def test_wordpress_connection(
    site_url: str, username: str, password: str, app_password: str = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if app_password:
            ...
            auth_header = base64.b64encode(
                f"{username}:{app_password}".encode()).decode()
...
def collect_wordpress_credentials() -> Dict[str, str]:
    ...
    credentials["SKYY_ROSE_PASSWORD"] = get_user_input(
        "WordPress Admin Password", sensitive=True)
...
def collect_ftp_credentials() -> Dict[str, str]:
    ...
    ftp_credentials["SKYY_ROSE_FTP_PASSWORD"] = get_user_input(
        "FTP Password", sensitive=True)
...
def collect_sftp_credentials() -> Dict[str, str]:
    ...
    auth_method = input(
        "Authentication method - (p)assword or (k)ey file? (p/k): ").lower()
    ...
    if auth_method.startswith("k"):
        ...
        sftp_credentials["SKYY_ROSE_SFTP_PASSWORD"] = get_user_input(
            "SFTP Password", sensitive=True
        )
    ...
    try:
        ...
        if test_result["success"]:
            ...
            continue_anyway = input(
                "Continue with setup anyway? (y/N): ").lower().startswith("y")
        ...
        print(
            f"✅ Application password: {'Yes' if wp_credentials.get('SKYY_ROSE_APP_PASSWORD') else 'No'}"
        )
