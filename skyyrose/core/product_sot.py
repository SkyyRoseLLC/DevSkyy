"""Compile the root-owned SkyyRose product sources into one machine contract.

Human-approved facts remain authored in ``data/skyyrose-catalog.csv``,
``data/dossiers/``, and ``data/logo-registry.json``. Downstream image, video,
website, advertising, and WooCommerce code reads the generated root manifest
instead of independently rejoining those sources.
"""
...
def resolve_reference(value: str) -> Path:
    """Resolve an explicit canonical reference without filename or SKU guessing."""
    reference = Path(value)
    candidates = (
        REPO_ROOT / reference,
        THEME_ROOT / reference,
        THEME_ROOT / "assets" / reference,
    )
...
def _evidence_kind(path: Path) -> str:
    ...
    if (
        "source-photos" in normalized
        or "/assets/products/references/" in normalized
        or "real-photo" in normalized
        or "real-" in path.name.lower()
    ) and path.suffix.lower() in {".jpg", ".jpeg", ".png", ".webp", ".avif"}:
...
def _proof_level(reference_stack: list[dict[str, Any]], media: dict[str, Any]) -> str:
    garment_references = [item for item in reference_stack if "garment_reference" in item["role"]]
...
def _branding_regions(schema: DossierSchema) -> list[dict[str, Any]]:
    """Serialize regions while adding material locks only where authored.

    Keeping the field absent on legacy regions avoids pretending those products
    already have material proof and prevents a nullable field from creating a
    misleading catalog-wide completion signal.
    """
    result: list[dict[str, Any]] = []
    for region in schema.branding:
        item = region.model_dump(mode="json", exclude={"material_lock"})
        if region.material_lock is not None:
            item["material_lock"] = region.material_lock.model_dump(mode="json")
        result.append(item)
    return result
...
def build_product(row: dict[str, str], registry: dict[str, Any]) -> dict[str, Any]:
    ...
    garment: dict[str, Any] = {
        ...
        "branding_regions": _branding_regions(schema),
        "negative_constraints": schema.negative,
    }
    if schema.material_lock_version:
        garment["material_lock_version"] = schema.material_lock_version
