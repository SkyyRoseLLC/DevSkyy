...
async def test_credential_loading():
    ...
    try:
        ...
        print(
            f"Environment validation: {'✅ VALID' if env_validation['valid'] else '❌ INVALID'}")
    ...
    try:
        ...
        try:
            ...
            if api_accessible:
                ...
                print(
                    f"❌ WordPress REST API not accessible (Status: {response.status_code})")
                return False
        ...
        if credentials.application_password:
            ...
            try:
                ...
                if auth_response.status_code == 200:
                    ...
                    print(
                        f"❌ Authentication failed (Status: {auth_response.status_code})")
    ...
    try:
        ...
        try:
            response = requests.get(
                f"{base_url}/api/v1/themes/credentials/status", timeout=5)
            if response.status_code == 200:
                data = response.json()
                print("✅ Credentials status endpoint working")
                print(f"   Configured sites: {data.get('configured_sites', [])}")
                print(
                    f"   Has default credentials: {data.get('has_default_credentials', False)}")
            else:
                print(
                    f"⚠️ Credentials status endpoint returned: {response.status_code}")
        ...
        try:
            response = requests.get(
                f"{base_url}/api/v1/themes/system-status", timeout=5)
            if response.status_code == 200:
                data = response.json()
                print("✅ System status endpoint working")
                print(
                    f"   Available theme types: {len(data.get('available_theme_types', []))}")
                print(
                    f"   Upload methods: {len(data.get('supported_upload_methods', []))}")
            else:
