...
class SkyRose3DPipeline:
    ...
    async def load_3d_model(
            ...
            brand_context: Optional[str] = None) -> Model3D:
    ...
    async def _extract_model_metadata(
        self, file_path: Path, model_format: ModelFormat
    ) -> Dict[str, Any]:
    ...
    async def _process_materials(
            self, file_path: Path, metadata: Dict[str, Any]) -> List[Material]:
    ...
    def _match_brand_attributes(
        self, metadata: Dict[str, Any], brand_context: Optional[str]
    ) -> List[str]:
    ...
    async def generate_360_view(
            self, model: Model3D, output_path: str) -> Dict[str, Any]:
